...
//
// The C code mainly aggregates all the functionality from previous
...
// routines implemented here, although it is quite generic and, as a
// result, very slow (expect this Perl/C/SPE implementation to be 10,
// 100, or more times faster than the Perl code on its own). The
// combination of the Perl program for high-level, non-critical code,
// and the C program for specialised, computation-heavy processing is
// a kind of demo in itself. Coding in C for SPEs takes a lot of
// development effort, and this division of labour shows one way
// (among many) of coding only the critical sections in C, while
// simultaneously achieving many of the benefits of using a high-level
// scripting language. So coding in this way can give you two kinds of
// speed: raw processing speed, and speed of development. (provided
// the interface between the two languages isn't too bad, of course; I
// think the mini language that interfaces the C and Perl here just
// about counts as acceptable).
//
...
// removed in favour of macro of same name
//const int verbose = 0;
