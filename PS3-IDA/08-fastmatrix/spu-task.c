...
// TODO: replace individual DMA get/put routines with single routine
...
int main(vec_ullong2 spe, vec_ullong2 argp, vec_ullong2 envp) {
  ...
  if (N_BUFS == 2) {
	...
	warm_multiply_u8(&xform, &in[cur], &out[cur], 
			 spu_splats((unsigned char)0x1b));
	...
	printf("SPE: waiting for input DMA transfer to complete\n");
      ...
      warm_multiply_u8(&xform, &in[slot], &out[slot], 
		       spu_splats((unsigned char)0x1b));
