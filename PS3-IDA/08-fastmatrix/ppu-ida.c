...
static int ida_init(codec_t *c) {
  ...
  // test task queue
  {
    ...
    for (i=0; i < EVENT_QUEUE_SIZE; ++i) {
      event_queue_push(c->task_queue, i, i);
      printf (" %d", i);
    }
    printf ("\nshifted out values:");
    for (i=0; i < EVENT_QUEUE_SIZE; ++i) {
      event_queue_shift(c->task_queue,&addr,&value);
      printf (" %d", addr);
    }
    printf ("\n");
  }
...
static int ida_process(codec_t *c) {
  ...
  //
  // End processing (copy/paste ... needs work)
  //

  // ping scheduler
  event_queue_push(c->task_queue, PING_SCHEDULER, 0);

  /*
  // tell SPE to read first block (fake message)
  message = 0;
  spe_in_mbox_write(c->run_context[0], &message, 1, SPE_MBOX_ALL_BLOCKING);
  */
