...
typedef struct {
  ...
  uint32_t poly;		// all bits of the irreducible field
                                ...
                                // routines). Choosing a polynomial
                                // existed in a previous incarnation
                                // of this code.
...
// Enumerate some things that can go wrong with call to ida_*.  Not
// all of these are actually checked for. Neither does the code
// currently do anything useful with this information, since the main
// error checking is done by the calling Perl program, which looks for
// printf statements starting with OK:, WARN: or ERROR: to determine
// if the slave program (this entire collection of C program files)
// works or not.
